...
4. For the program to give the user opportunity to do a series of calculations within the same boot,
   ...
   'ZeroDivisionError' or 'ValueError' in a friendly manner, within a 'while' loop.
    4a. The variable 'file' with the value 'a' opens a new file and records every operation therein.
    4b. The latter three variables are for the user's input which later is used to execute 
        an operation.
    ...
    4d. In the next four 'if' conditional blocks, the operations for particular types of equations 
        are performed. 
        Each block consists of the following statements:
        - 'print()' statement to display the result of the equation;
        - '.write()' statement to write the result into the text file;
        - user input statement to check whether or not they want to continue;
        ...
        - the steps as above constitute each of the 'if' conditional blocks.
        - I wrapped with a 'round()' function the result of the division within the division 
          'if' block to control the decimal points. The results are rounded to one decimal point.
    4e. At the end of the loop, a '.close()' statement closes the file, and no more results are 
...
5. To get the "read" stage to work well and make it user-friendly, the try-except block within 
   the while loop gives again user the chance to attempt a correct input as long as they fail.
    ...
    5c. The 'print()' statement will display them all, and the last three will terminate 
        the program.
6. I tested my application in numerous ways to see if it has any bugs, and I haven't found 
