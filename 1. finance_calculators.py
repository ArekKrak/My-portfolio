...
3. Before I proceeded, I created a 'try-except' block that will throw a 'KeyboardInterrupt' 
   exception at any point in the event of interrupting the application by the user.
...
5. I set a 'while' loop to check if a user enters a valid value; if not, the error message
   will pop up, and they will be prompted to try again until they succeed every time they 
   mistype the answer.
6. I've set a block of nested 'if' conditions to perform the following operations:
    ...
    6c. Of course, if the user mistypes the answer, they will receive an error, after which 
        they will be prompted to try again, in the same way as in point 4.
    6d. The variables containing all calculations were set, as given in the 'Interest formula'.
...
8. I included the whole 'ValueError'-vulnerable code within the 'try-except' block to make sure
   ...
   within the current run.
    8a. At the end of each 'if' block, I added a 'break' statement, as, without it, the application
